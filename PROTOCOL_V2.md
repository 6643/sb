...
> 状态: 设计草案, Go / TypeScript 侧都已实现独立 runtime 与生成原型 (`go/sb/v2`, `go/sbv2`, `ts/sbv2`), 但未切换为默认协议  
> 兼容性: 与当前 `SB` 实现不兼容  
> 目的: 讨论下一版更紧凑的 wire format, 不是描述当前默认生成器行为
...
Go 与 TypeScript 运行时都需要先补齐以下基础能力:
...
生成器需要从“当前 presence bit + 固定长度前缀”切到 v2 规则:
...
1. 先做运行时 bitstream 与 `u24`
2. 再做单字段 `text/bin/list` 的 v2 编解码
3. 再做 struct 的 `header + fixed block + var tail`
4. 再做 list 的三类路径
5. 最后补 benchmark 和文档回归

### 13.6 实现边界

因为 v2 与当前实现不兼容, 落地时更稳的做法是:

- 单独放在 v2 runtime / v2 generator 路径
- 不要在现有 v1 运行时里就地替换
- 等 v2 自测稳定后, 再决定是否暴露为生成开关

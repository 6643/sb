...
## 下一步验证建议
- 补齐 Go 工具链后执行: `go test ./...`.
- 执行联合验证脚本: `./test/run_go_bun_tests.sh`.
- 若失败, 按“关键回归测试索引”定位对应规则并回归修复.

## 完成判定
- 验证结论: PEG 与 lexer/parser 的核心语法规则已完成对齐.
- 测试清单: 关键回归测试已在本文档建立索引.
- 风险清单: 当前环境缺少 Go 工具链, 自动化验证待环境恢复后执行.
- 变更记录: 所有原子改动均已写入 `CHANGELOG.md` 顶部条目.

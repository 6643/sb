...
export const getAccountStatusListBody = (buf: rt.Buffer, state: number): [AccountStatus[], Error | null] =>
    rt.getBitmapListCompact<AccountStatus>(
        buf,
        state,
        () => DefaultAccountStatus(),
        (buf) => {
            const [value, err] = rt.getU8(buf);
            if (err !== null) return [DefaultAccountStatus(), err];
...
export const setAccountStatusListBody = (buf: rt.Buffer, state: number, v: AccountStatus[]): Error | null =>
    rt.setBitmapListCompact<AccountStatus>(
        buf,
        state,
        v,
...
export const getTypeListBody = (buf: rt.Buffer, state: number): [Type[], Error | null] =>
    rt.getBitmapListCompact<Type>(
        buf,
        state,
        () => DefaultType(),
        (buf) => {
            const [value, err] = rt.getU8(buf);
            if (err !== null) return [DefaultType(), err];
...
export const setTypeListBody = (buf: rt.Buffer, state: number, v: Type[]): Error | null =>
    rt.setBitmapListCompact<Type>(
        buf,
        state,
        v,
...
export const getStatusListBody = (buf: rt.Buffer, state: number): [Status[], Error | null] =>
    rt.getBitmapListCompact<Status>(
        buf,
        state,
        () => DefaultStatus(),
        (buf) => {
            const [value, err] = rt.getU8(buf);
            if (err !== null) return [DefaultStatus(), err];
...
export const setStatusListBody = (buf: rt.Buffer, state: number, v: Status[]): Error | null =>
    rt.setBitmapListCompact<Status>(
        buf,
        state,
        v,
...
export const getStatusAListBody = (buf: rt.Buffer, state: number): [StatusA[], Error | null] =>
    rt.getBitmapListCompact<StatusA>(
        buf,
        state,
        () => DefaultStatusA(),
        (buf) => {
            const [value, err] = rt.getU8(buf);
            if (err !== null) return [DefaultStatusA(), err];
...
export const setStatusAListBody = (buf: rt.Buffer, state: number, v: StatusA[]): Error | null =>
    rt.setBitmapListCompact<StatusA>(
        buf,
        state,
        v,
...
export const getItemStatusListBody = (buf: rt.Buffer, state: number): [ItemStatus[], Error | null] =>
    rt.getBitmapListCompact<ItemStatus>(
        buf,
        state,
        () => DefaultItemStatus(),
        (buf) => {
            const [value, err] = rt.getU8(buf);
            if (err !== null) return [DefaultItemStatus(), err];
...
export const setItemStatusListBody = (buf: rt.Buffer, state: number, v: ItemStatus[]): Error | null =>
    rt.setBitmapListCompact<ItemStatus>(
        buf,
        state,
        v,
...
export const getSimPickPhoneListBody = (buf: rt.Buffer, state: number): [SimPickPhone[], Error | null] =>
    rt.getBitmapListCompact<SimPickPhone>(
        buf,
        state,
        () => DefaultSimPickPhone(),
        (buf) => {
            const [value, err] = rt.getU8(buf);
            if (err !== null) return [DefaultSimPickPhone(), err];
...
export const setSimPickPhoneListBody = (buf: rt.Buffer, state: number, v: SimPickPhone[]): Error | null =>
    rt.setBitmapListCompact<SimPickPhone>(
        buf,
        state,
        v,
...
export const getSimOperatorListBody = (buf: rt.Buffer, state: number): [SimOperator[], Error | null] =>
    rt.getBitmapListCompact<SimOperator>(
        buf,
        state,
        () => DefaultSimOperator(),
        (buf) => {
            const [value, err] = rt.getU8(buf);
            if (err !== null) return [DefaultSimOperator(), err];
...
export const setSimOperatorListBody = (buf: rt.Buffer, state: number, v: SimOperator[]): Error | null =>
    rt.setBitmapListCompact<SimOperator>(
        buf,
        state,
        v,
...
export const getOrderStatusListBody = (buf: rt.Buffer, state: number): [OrderStatus[], Error | null] =>
    rt.getBitmapListCompact<OrderStatus>(
        buf,
        state,
        () => DefaultOrderStatus(),
        (buf) => {
            const [value, err] = rt.getU8(buf);
            if (err !== null) return [DefaultOrderStatus(), err];
...
export const setOrderStatusListBody = (buf: rt.Buffer, state: number, v: OrderStatus[]): Error | null =>
    rt.setBitmapListCompact<OrderStatus>(
        buf,
        state,
        v,

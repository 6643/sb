...
func TestGoGeneratorWritesSchemaFile(t *testing.T) {
	...
	assertContains(t, text, "return rt.GetBitmapPtrListCompactInto(buf, state, dst, defaultSimInfo, GetSimInfo)")
	assertContains(t, text, "return rt.SetBitmapPtrListCompactValidated(")
	assertContains(t, text, "\"SimInfo\",")
	assertContains(t, text, "validateSimInfo,")
	assertContains(t, text, "return rt.SizeBitmapPtrListCompactValidated(")
	assertContains(t, text, "fieldSizeTitles, err := rt.SizeTextListCompact(s.Titles)")
	assertContains(t, text, "fieldSizeTags, err := rt.SizeZeroFixedListCompact(s.Tags, 4)")
	assertContains(t, text, "valueTags, err := rt.GetZeroFixedListCompactInto(buf, tagsState, reuseTags, rt.GetU32)")
	assertContains(t, text, "if err := rt.SetZeroFixedListCompactSized(buf, tagsState, s.Tags, 4, rt.SetU32); err != nil { return fmt.Errorf(\"SetSim Tags: %w\", err) }")

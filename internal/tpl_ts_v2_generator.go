...
type TsV2Generator struct {
	Config Config
	base   *TsGenerator
}

func NewTsV2Generator(cfg Config) *TsV2Generator {
	return &TsV2Generator{
		Config: cfg,
		base:   NewTsGenerator(cfg),
	}
}

func (g *TsV2Generator) Generate(schema *TplSchema) error {
	targetDir := filepath.Join(g.Config.TsDir, "sbv2")
	...
	files := []generatedFile{
		{RelativePath: "type.ts", Data: []byte(renderTsV2RuntimeSource()), Perm: 0644},
...
func (g *TsV2Generator) removeLegacyAPIWrapperStructFiles(targetDir string) error {
	patterns := []string{
...
func (g *TsV2Generator) renderEnumFile(enums []TplEnum) string {
	var w sourceWriter
	...
	for _, enum := range enums {
		...
		w.Blank()
		w.Linef("export const get%sListBody = (buf: rt.Buffer, state: number): [%s[], Error | null] =>", enumName, enumName)
		w.Linef("    rt.getBitmapListCompact<%s>(", enumName)
		w.Line("        buf,")
		w.Line("        state,")
		w.Linef("        () => Default%s(),", enumName)
		w.Line("        (buf) => {")
		w.Line("            const [value, err] = rt.getU8(buf);")
		w.Linef("            if (err !== null) return [Default%s(), err];", enumName)
		w.Linef("            const item = value as %s;", enumName)
		w.Linef("            if (!Is%s(item)) return [Default%s(), new Error(`非法枚举值: ${item}`)];", enumName, enumName)
		w.Line("            return [item, null];")
		w.Line("        },")
		w.Line("    );")
		w.Blank()
		w.Linef("export const set%sListBody = (buf: rt.Buffer, state: number, v: %s[]): Error | null =>", enumName, enumName)
		w.Linef("    rt.setBitmapListCompact<%s>(", enumName)
		w.Line("        buf,")
		w.Line("        state,")
		w.Line("        v,")
...
func (g *TsV2Generator) renderStructFile(st TplStruct) string {
	...
	w.Line("        if (err === null) Object.assign(s, next);")
	w.Line("        return err;")
	w.Line("    };")
	w.Line("    return s;")
	w.Line("};")
	...
	w.Line("};")
	w.Blank()
	w.Linef("export const validate%s = (s: %s | null | undefined): Error | null => {", name, name)
	w.Line("    if (s === null || s === undefined) return null;")
	for _, field := range st.Fields {
		...
		case field.Type.Kind == TplKindStruct && !field.Type.IsList:
			w.Linef("    { const err = _.validate%s(%s); if (err !== null) return new Error(`%s: ${err.message}`); }", PascalCase(field.Type.Name), ref, fieldName)
		case field.Type.Kind == TplKindStruct && field.Type.IsList:
			w.Linef("    { const [, err] = rt.listCountState(%s.length); if (err !== null) return new Error(`%s: ${err.message}`); }", ref, fieldName)
			w.Linef("    for (let i = 0; i < %s.length; i++) {", ref)
	...
	w.Line("};")
	w.Blank()
	w.Linef("export const get%s = (buf: rt.Buffer): [%s, Error | null] => {", name, name)
	...
	w.Linef("    const errPadding = rt.validatePaddingZero(header, headerBits, \"%s header\");", name)
	w.Line("    if (errPadding !== null) return [s, errPadding];")
	w.Line("    const reader = new rt.BitReader(header, headerBits);")
	for _, field := range st.Fields {
		fieldName := CamelCase(PascalCase(field.Name))
		switch g.v2TagWidth(field.Type) {
		case 1:
			if field.Type.Name == "bool" {
				w.Linef("    const [%sState, err%sState] = reader.readBit();", fieldName, PascalCase(fieldName))
			} else {
				w.Linef("    const [%sPresent, err%sPresent] = reader.readBit();", fieldName, PascalCase(fieldName))
			}
		case 2:
			w.Linef("    const [%sState, err%sState] = reader.readBits(2);", fieldName, PascalCase(fieldName))
		}
		w.Linef("    if (err%s%s !== null) return [s, new Error(`get %s %s header: ${err%s%s.message}`)];", PascalCase(fieldName), g.readErrSuffix(field.Type), name, PascalCase(field.Name), PascalCase(fieldName), g.readErrSuffix(field.Type))
	}
	for _, field := range st.Fields {
		fieldName := PascalCase(field.Name)
		...
		case field.Type.Kind == TplKindStruct && field.Type.IsList:
			w.Linef("    { const [value, err] = _.get%sListBody(buf, %sState); if (err !== null) return [s, new Error(`get %s %s: ${err.message}`)]; %s = value; }", PascalCase(field.Type.Name), stateVar, name, fieldName, fieldVar)
		case field.Type.Kind == TplKindEnum && field.Type.IsList:
			w.Linef("    { const [value, err] = _.get%sListBody(buf, %sState); if (err !== null) return [s, new Error(`get %s %s: ${err.message}`)]; %s = value; }", PascalCase(field.Type.Name), stateVar, name, fieldName, fieldVar)
	...
	w.Line("    if (errValidate !== null) return [s, new Error(`validate failed: ${errValidate.message}`)];")
	w.Line("    return [s, null];")
	w.Line("};")
	w.Blank()
	w.Linef("export const set%s = (buf: rt.Buffer, s: %s): Error | null => {", name, name)
	w.Linef("    if (s === null || s === undefined) return new Error(`set %s: value is null or undefined`);", name)
	w.Linef("    const errValidate = validate%s(s);", name)
	w.Linef("    if (errValidate !== null) return new Error(`validate %s: ${errValidate.message}`);", name)
	...
	w.Linef("    const header = new rt.BitWriter(%d);", headerBits)
	for _, field := range st.Fields {
		fieldName := PascalCase(field.Name)
		ref := "s." + CamelCase(field.Name)
		switch {
		case field.Type.Name == "bool":
			w.Linef("    header.writeBit(%s);", ref)
		case g.v2TagWidth(field.Type) == 1:
			w.Linef("    header.writeBit(%s);", g.nonDefaultExpr(field.Type, ref))
		default:
			w.Linef("    { const err = header.writeBits(%sState, 2); if (err !== null) { buf.rewindWrite(startOffset); return new Error(`set %s %s header: ${err.message}`); } }", CamelCase(fieldName), name, fieldName)
		}
	}
	w.Line("    const errHeaderWrite = buf.write(header.bytes);")
	...
	for _, field := range st.Fields {
		...
		case field.Type.Kind == TplKindStruct && field.Type.IsList:
			w.Linef("    { const err = _.set%sListBody(buf, %sState, %s); if (err !== null) { buf.rewindWrite(startOffset); return new Error(`set %s %s: ${err.message}`); } }", PascalCase(field.Type.Name), CamelCase(fieldName), ref, name, fieldName)
		case field.Type.Kind == TplKindEnum && field.Type.IsList:
			w.Linef("    { const err = _.set%sListBody(buf, %sState, %s); if (err !== null) { buf.rewindWrite(startOffset); return new Error(`set %s %s: ${err.message}`); } }", PascalCase(field.Type.Name), CamelCase(fieldName), ref, name, fieldName)
		case field.Type.Kind == TplKindStruct:
	...
	w.Line("};")
	w.Blank()
	w.Linef("export const read%s = (buf: rt.Buffer): [%s, Error | null] => get%s(buf);", name, name, name)
	w.Blank()
...
func (g *TsV2Generator) renderStructListBodyHelpers(w *sourceWriter, st TplStruct) {
	name := PascalCase(st.Name)
	w.Linef("export const get%sListBody = (buf: rt.Buffer, state: number): [%s[], Error | null] =>", name, name)
	w.Linef("    rt.getBitmapListCompact<%s>(", name)
	w.Line("        buf,")
	w.Line("        state,")
	w.Linef("        () => new%s(),", name)
	...
	w.Line("    );")
	w.Blank()
	w.Linef("export const set%sListBody = (buf: rt.Buffer, state: number, v: %s[]): Error | null =>", name, name)
	w.Linef("    rt.setBitmapListCompact<%s>(", name)
	w.Line("        buf,")
	w.Line("        state,")
	w.Line("        v,")
...
func (g *TsV2Generator) renderIndexFile(files []string) string {
	var w sourceWriter
...
func (g *TsV2Generator) renderRPCFile(apis []TplApi) string {
	var w sourceWriter
	w.Line("import * as _ from \"./_\"")
	w.Blank()
	...
	for _, api := range apis {
		...
		for _, arg := range api.Args {
			g.tsV2DirectWrite(&w, arg.Type, CamelCase(arg.Name), "buf", g.rpcReqErrReturn(api.Result, defaultVal))
		}
		...
		g.tsV2DirectRead(&w, api.Result, "result", "respBuf", fmt.Sprintf("[%s, RpcErrCode.RespErr]", defaultVal))
...
func (g *TsV2Generator) renderSmokeTest(apis []TplApi) string {
	var w sourceWriter
...
func (g *TsV2Generator) renderEnumSmokeTest(enums []TplEnum) string {
	var w sourceWriter
	w.Line("import { describe, expect, test } from \"bun:test\";")
	w.Blank()
	w.Line("import * as _ from \"./_\";")
...
func (g *TsV2Generator) fieldType(t TplType) string {
	base := g.base.getTsType(TplType{Name: t.Name, Kind: t.Kind})
	switch t.Kind {
...
func (g *TsV2Generator) defaultValue(t TplType) string {
	if t.IsList {
		return "[]"
	}
	switch t.Kind {
	case TplKindBase:
		return g.base.getTsValue(t.Name)
	case TplKindEnum:
...
func (g *TsV2Generator) v2HeaderBits(st TplStruct) int {
	total := 0
	for _, field := range st.Fields {
...
func (g *TsV2Generator) v2TagWidth(t TplType) int {
...
func (g *TsV2Generator) nonDefaultExpr(t TplType, ref string) string {
	switch {
	case t.IsList:
...
func (g *TsV2Generator) primitiveGetter(name string) string {
	switch name {
	case "i8":
		return "getI8"
	case "u8":
...
func (g *TsV2Generator) primitiveSetter(name string) string {
	switch name {
	case "i8":
		return "setI8"
	case "u8":
...
func (g *TsV2Generator) primitiveDefault(name string) string {
	switch name {
	case "i64", "u64":
		return "0n"
	default:
		return g.base.getTsValue(name)
	}
}

func (g *TsV2Generator) primitiveEq(name string) string {
	switch name {
	case "i8":
		return "rt.eqI8"
	case "u8":
...
func (g *TsV2Generator) listGetExpr(t TplType, stateVar string) string {
	switch t.Name {
	case "bool":
	...
	default:
		return fmt.Sprintf("rt.getBitmapListCompact<%s>(buf, %s, %s, %s)", g.base.getTsType(TplType{Name: t.Name, Kind: t.Kind}), stateVar, g.bitmapDefaultFactory(t), g.bitmapGetter(t))
	}
}

func (g *TsV2Generator) listSetExpr(t TplType, ref, stateVar string) string {
	switch t.Name {
	case "bool":
	...
	default:
		return fmt.Sprintf("rt.setBitmapListCompact<%s>(buf, %s, %s, %s, %s)", g.base.getTsType(TplType{Name: t.Name, Kind: t.Kind}), stateVar, ref, g.bitmapIsDefault(t), g.bitmapSetter(t))
	}
}

func (g *TsV2Generator) bitmapDefaultFactory(t TplType) string {
	switch t.Kind {
	case TplKindEnum:
		return fmt.Sprintf("() => _.Default%s()", PascalCase(t.Name))
	case TplKindStruct:
...
func (g *TsV2Generator) bitmapIsDefault(t TplType) string {
	switch t.Kind {
	case TplKindEnum:
...
func (g *TsV2Generator) bitmapGetter(t TplType) string {
	switch t.Kind {
	case TplKindEnum:
		return fmt.Sprintf("(buf) => { const [value, err] = rt.getU8(buf); if (err !== null) return [_.Default%s(), err]; const item = value as _.%s; if (!_.Is%s(item)) return [_.Default%s(), new Error(`非法枚举值: ${item}`)]; return [item, null]; }", PascalCase(t.Name), PascalCase(t.Name), PascalCase(t.Name), PascalCase(t.Name))
	case TplKindStruct:
		return fmt.Sprintf("(buf) => _.read%s(buf)", PascalCase(t.Name))
	default:
		return fmt.Sprintf("(buf) => rt.%s(buf)", g.primitiveGetter(t.Name))
	}
}

func (g *TsV2Generator) bitmapSetter(t TplType) string {
	switch t.Kind {
	case TplKindEnum:
	...
	default:
		return fmt.Sprintf("(buf, item) => rt.%s(buf, item as any)", g.primitiveSetter(t.Name))
	}
}

func (g *TsV2Generator) eqExpr(t TplType, left, right string) string {
	switch {
...
func (g *TsV2Generator) rpcArgList(args []TplApiArg) string {
	...
	for _, arg := range args {
		...
		parts = append(parts, fmt.Sprintf("%s: %s%s", CamelCase(arg.Name), prefix, g.base.getTsLogicType(arg.Type)))
	}
	return strings.Join(parts, ", ")
}

func (g *TsV2Generator) rpcPromiseType(result TplType) string {
	if result.Name == "nil" {
		return "RpcErrCode"
	}
	prefix := ""
	...
	return fmt.Sprintf("[%s%s, RpcErrCode]", prefix, g.base.getTsLogicType(result))
}

func (g *TsV2Generator) rpcDefaultValue(result TplType) string {
	if result.IsList {
		return "[]"
	}
	switch result.Kind {
	case TplKindBase:
		return g.base.getTsValue(result.Name)
	case TplKindEnum:
...
func (g *TsV2Generator) rpcReqErrReturn(result TplType, defaultVal string) string {
	if result.Name == "nil" {
...
func (g *TsV2Generator) rpcStatusReturn(result TplType, defaultVal string) string {
	if result.Name == "nil" {
		return "status"
	}
...
func (g *TsV2Generator) v2APIReqStruct(api TplApi) (TplStruct, bool) {
	if len(api.Args) == 0 {
...
func (g *TsV2Generator) v2APIRespStruct(api TplApi) (TplStruct, bool) {
	if api.Result.Name == "nil" {
		return TplStruct{}, false
	}
	return TplStruct{
...
func (g *TsV2Generator) v2APIReqTypeName(api TplApi) (string, bool) {
...
func (g *TsV2Generator) v2APIRespTypeName(api TplApi) (string, bool) {
...
func (g *TsV2Generator) tsV2DirectRead(w *sourceWriter, t TplType, target, bufVar, errReturn string) {
	w.Line("        {")
	switch {
	...
	case t.IsList:
		w.Linef("            const [state, errState] = _.getU8(%s);", bufVar)
		w.Linef("            if (errState !== null) return %s;", errReturn)
		w.Linef("            const [value, err] = %s;", g.tsV2DirectListGetExpr(t, "state", bufVar))
		w.Linef("            if (err !== null) return %s;", errReturn)
		w.Linef("            %s = value as any;", target)
	case t.Kind == TplKindStruct:
...
func (g *TsV2Generator) tsV2DirectWrite(w *sourceWriter, t TplType, ref, bufVar, errReturn string) {
	w.Line("        {")
	switch {
	case t.Name == "bool" && !t.IsList:
	...
	case t.IsList:
		...
		w.Linef("            const err0 = _.setU8(%s, state);", bufVar)
		w.Linef("            if (err0 !== null) return %s;", errReturn)
		w.Linef("            const err1 = %s;", g.tsV2DirectListSetExpr(t, ref, "state", bufVar))
		w.Linef("            if (err1 !== null) return %s;", errReturn)
	case t.Kind == TplKindStruct:
...
func (g *TsV2Generator) tsV2DirectListGetExpr(t TplType, stateVar, bufVar string) string {
	switch {
	case t.Name == "bool":
	...
	case t.Kind == TplKindBase:
		return fmt.Sprintf("_.getBitmapListCompact<%s>(%s, %s, %s, %s)", g.base.getTsType(TplType{Name: t.Name, Kind: t.Kind}), bufVar, stateVar, g.bitmapDefaultFactory(t), g.bitmapGetter(t))
	case t.Kind == TplKindEnum:
...
func (g *TsV2Generator) tsV2DirectListSetExpr(t TplType, ref, stateVar, bufVar string) string {
	switch {
	case t.Name == "bool":
	...
	case t.Kind == TplKindBase:
		return fmt.Sprintf("_.setBitmapListCompact<%s>(%s, %s, %s, %s, %s)", g.base.getTsType(TplType{Name: t.Name, Kind: t.Kind}), bufVar, stateVar, ref, g.bitmapIsDefault(t), g.bitmapSetter(t))
	case t.Kind == TplKindEnum:
...
func (g *TsV2Generator) readErrSuffix(t TplType) string {
	if t.Name == "bool" && !t.IsList {
		return "State"
	}
	if g.v2TagWidth(t) == 1 {

...
func TestParseEnumHeaderCommentAccepted(t *testing.T) {
	input := `// 状态定义
Status = Ok(0) | Err(1)`
	l := New(input)
	p := NewParser(l)
	...
	if s.Enums[0].Note != "状态定义" {
		t.Fatalf("unexpected enum note: %q", s.Enums[0].Note)
	}
...
func TestParseStructCommentLineBindsFieldHeadNote(t *testing.T) {
	input := `User {
// 第一行
// 第二行
id u32
	...
	if s.Structs[0].Fields[0].Note != "第一行\n第二行" {
		t.Fatalf("unexpected field note: %q", s.Structs[0].Fields[0].Note)
	}
}

func TestParseStructEmbeddedFieldWithEllipsisAccepted(t *testing.T) {
	input := `User {
id u32
}
Admin {
...
role text
	...
	fields := s.Structs[1].Fields
	if len(fields) != 2 {
		t.Fatalf("unexpected field count: %d", len(fields))
	}
	if !fields[0].Embedded || fields[0].Type.Name != "User" || fields[0].Name != "" {
		t.Fatalf("unexpected embedded field: %+v", fields[0])
	}
}

func TestParseStructEmbeddedFieldWithoutEllipsisRejected(t *testing.T) {
	input := `User {
id u32
}
Admin {
...
role text
	...
	if !strings.Contains(err.Error(), "字段 User 缺少类型") {
		t.Fatalf("unexpected error: %v", err)
	}
}

func TestParseStructInlineFieldCommentRejected(t *testing.T) {
	input := `User {
id u32 // inline
}`
	l := New(input)
	...
	if !strings.Contains(err.Error(), "字段注释必须独占一行并写在字段前") {
		t.Fatalf("unexpected error: %v", err)
	}
}

func TestParseStructEmbeddedFieldEllipsisRequiresTypeName(t *testing.T) {
	input := `Admin {
...
role text
}`
	...
	if !strings.Contains(err.Error(), "嵌入字段必须写为 ...TypeName") {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestParseEnumMemberHeadCommentRejected(t *testing.T) {
	input := `Status =
// 第一行
// 第二行
Ok(0)
| Err(1)`
	...
	if !strings.Contains(err.Error(), "枚举仅支持定义前整体注释, 不支持成员注释") {
		t.Fatalf("unexpected error: %v", err)
	}
}

func TestParseEnumInlineMemberCommentRejected(t *testing.T) {
	input := `Status = Ok(0) // inline`
	l := New(input)
	p := NewParser(l)
	_, err := p.ParseSchema()
	if err == nil {
...
func TestParseEnumCommentAfterPipeRejected(t *testing.T) {
	input := `Status = Ok(0) |
// 成员注释
Err(1)`
	l := New(input)

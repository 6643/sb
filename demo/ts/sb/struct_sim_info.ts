...
export const isZeroSimInfo = (s: SimInfo | null | undefined): boolean => {
    ...
    if (!rt.isStringValue(s.title) || s.title !== "") return false;
    if (!rt.isStringValue(s.content) || s.content !== "") return false;
    if (s.a !== false) return false;
    if (s.b !== false) return false;
    if (s.c !== false) return false;
    if (s.d !== false) return false;
    if (!rt.isBinValue(s.zip) || s.zip.byteLength !== 0) return false;
    return true;
};
...
export const validateSimInfo = (s: SimInfo | null | undefined): rt.Err => {
    ...
    if (!rt.isBinValue(s.zip)) return new Error(`Zip: value is not Uint8Array`);

...
const rechargeAMeta = rm.defineStruct<RechargeA>({
    name: "RechargeA",
    ...
    fields: [
        ...
        rm.scalarField<RechargeA, number>("id", "Id", 0, rt.getU32, rt.setU32, rt.eqU32),
        rm.defaultListField<RechargeA, OrderStatus>("type", "Type", DefaultOrderStatus, getOrderStatusListBody, setOrderStatusListBody, (item) => IsAssignableOrderStatus(item as any) ? undefined : new Error(`非法枚举值: ${item as any}`), (item) => IsDefaultOrderStatus(item as any), (left, right) => NormalizeOrderStatus(left as any) === NormalizeOrderStatus(right as any)),
        rm.textListField<RechargeA>("phone", "Phone"),
        rm.structField<RechargeA, SimInfo>("si", "Si", isZeroSimInfo, readSimInfo, setSimInfo, validateSimInfo, eqSimInfo),
        rm.scalarField<RechargeA, number>("aid", "Aid", 0, rt.getU32, rt.setU32, rt.eqU32),
    ],
});

export const newRechargeA = (): RechargeA => rm.newStruct(rechargeAMeta, getRechargeA, setRechargeA);
export const isZeroRechargeA = (s: RechargeA | null | undefined): boolean => rm.isZeroStruct(rechargeAMeta, s);
export const validateRechargeA = (s: RechargeA | null | undefined): rt.Err => rm.validateStruct(rechargeAMeta, s);
export const getRechargeA = (buf: rt.Buffer): [RechargeA, rt.Err] => rm.getStruct(rechargeAMeta, buf);
export const setRechargeA = (buf: rt.Buffer, s: RechargeA): rt.Err => rm.setStruct(rechargeAMeta, buf, s);
export const readRechargeA = (buf: rt.Buffer): [RechargeA, rt.Err] => getRechargeA(buf);
export const eqRechargeA = (a: RechargeA | null | undefined, b: RechargeA | null | undefined): boolean => rm.eqStruct(rechargeAMeta, a, b);

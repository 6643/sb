...
const _textTypeError = (v: unknown): Error => new Error(`text value must be string: ${typeof v}`);
...
const _validateTextValue = (v: string): [Uint8Array, Error | null] => {
    if (!isStringValue(v)) return [new Uint8Array(0), _textTypeError(v)];
...
export const textState = (v: string): [number, Error | null] => {
    if (!isStringValue(v)) return [0, _textTypeError(v)];
    return _textLengthState(_textBytes(v).byteLength);
};

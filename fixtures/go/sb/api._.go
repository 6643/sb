...
func UserGetAbcHandler(w http.ResponseWriter, r *http.Request) {
	...
	{
		if !isAssignableOrderStatus(result) {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		if err := SetU8(&body, uint8(normalizeOrderStatus(result))); err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
	}
...
func GetCountHandler(w http.ResponseWriter, r *http.Request) {
	...
	var body bytes.Buffer
	{
		if err := SetU8(&body, result); err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
	}
...
func GetBinHandler(w http.ResponseWriter, r *http.Request) {
	...
	var body bytes.Buffer
	{
		...
		if err := SetU8(&body, state); err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		if err := SetBin(&body, state, result); err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
	}
...
func checkStatus(w http.ResponseWriter, status RpcErrCode) bool {
	...
	w.WriteHeader(httpStatusFromRPC(status))
	return false
}
...
func parseRequest(w http.ResponseWriter, r *http.Request) (*bytes.Buffer, bool) {
	...
	if int64(len(body)) > defaultMaxReqBytes {

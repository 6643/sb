...
export class RpcClient {
    ...
    /**  */
    public submit = async (m: _.Matrix): Promise<RpcErrCode> => {
        const buf = new _.Buffer();
        if (_.setAll(buf, (buf) => _.setMatrix(buf, m as any)) !== null) return RpcErrCode.ReqErr;

        const [bytes, status] = await this._fetch("submit", buf.bytes);
    ...
    /**  */
    public fetch = async (): Promise<[_.Matrix, RpcErrCode]> => {
        const buf = new _.Buffer();

        const [bytes, status] = await this._fetch("fetch", buf.bytes);
        if (status !== RpcErrCode.Ok || bytes === null) return [_.newMatrix(), status];

        const respBuf = new _.Buffer(bytes);const [result, err] = _.getMatrix(respBuf);
        if (err !== null) return [_.newMatrix(), RpcErrCode.RespErr];
        if (respBuf.len !== 0) return [_.newMatrix(), RpcErrCode.RespErr];

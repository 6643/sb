...
// CallSubmit 
func CallSubmit(c *Client, ctx context.Context, m *Matrix) (errCode RpcErrCode) {
	var buf bytes.Buffer
	if m == nil {
		return RpcReqErr
	}
	if err := SetAll(&buf, func(buf *bytes.Buffer) error {
		return SetMatrix(buf, m)
	}); err != nil {
		return RpcReqErr
	}

	body, status := doClient(c, ctx, "/submit", buf.Bytes())
	if status != RpcOk {
		return status
	}
...
// CallFetch 
func CallFetch(c *Client, ctx context.Context) (result *Matrix, errCode RpcErrCode) {
	var buf bytes.Buffer

	body, status := doClient(c, ctx, "/fetch", buf.Bytes())
	if status != RpcOk {

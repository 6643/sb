...
// CallRound 
func CallRound(c *Client, ctx context.Context, payload *BlobWrap) (result *BlobWrap, errCode RpcErrCode) {
	var buf bytes.Buffer
	if payload == nil {
		return result, RpcReqErr
	}
	if err := SetAll(&buf, func(buf *bytes.Buffer) error {
		return SetBlobWrap(buf, payload)
	}); err != nil {
		return result, RpcReqErr
	}

	body, status := doClient(c, ctx, "/round", buf.Bytes())
	if status != RpcOk {

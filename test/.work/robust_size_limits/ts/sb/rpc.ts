...
export class RpcClient {
    ...
    /**  */
    public round = async (payload: _.BlobWrap): Promise<[_.BlobWrap, RpcErrCode]> => {
        const buf = new _.Buffer();
        if (_.setAll(buf, (buf) => _.setBlobWrap(buf, payload as any)) !== null) return [_.newBlobWrap(), RpcErrCode.ReqErr];

        const [bytes, status] = await this._fetch("round", buf.bytes);
        if (status !== RpcErrCode.Ok || bytes === null) return [_.newBlobWrap(), status];

        const respBuf = new _.Buffer(bytes);const [result, err] = _.getBlobWrap(respBuf);
        if (err !== null) return [_.newBlobWrap(), RpcErrCode.RespErr];
        if (respBuf.len !== 0) return [_.newBlobWrap(), RpcErrCode.RespErr];

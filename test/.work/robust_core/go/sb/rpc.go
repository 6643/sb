...
// CallEcho 
func CallEcho(c *Client, ctx context.Context, env *Envelope) (result *Envelope, errCode RpcErrCode) {
	var buf bytes.Buffer
	if env == nil {
		return result, RpcReqErr
	}
	if err := SetAll(&buf, func(buf *bytes.Buffer) error {
		return SetEnvelope(buf, env)
	}); err != nil {
		return result, RpcReqErr
	}

	body, status := doClient(c, ctx, "/echo", buf.Bytes())
	if status != RpcOk {
...
// CallEchoTail 
func CallEchoTail(c *Client, ctx context.Context, env *Envelope) (result *Envelope, errCode RpcErrCode) {
	var buf bytes.Buffer
	if env == nil {
		return result, RpcReqErr
	}
	...
	body, status := doClient(c, ctx, "/echo_tail", buf.Bytes())
	if status != RpcOk {
		return result, status
	}
...
// CallGetColor 
func CallGetColor(c *Client, ctx context.Context) (result Color, errCode RpcErrCode) {
	var buf bytes.Buffer

	body, status := doClient(c, ctx, "/get_color", buf.Bytes())
	if status != RpcOk {
		return result, status
	}
	...
	if respBuf.Len() != 0 { return result, RpcRespErr }
	if !IsColor(result) { return result, RpcRespErr }
	return result, status
}
// CallGetBadColor 
func CallGetBadColor(c *Client, ctx context.Context) (result Color, errCode RpcErrCode) {
	var buf bytes.Buffer

	body, status := doClient(c, ctx, "/get_bad_color", buf.Bytes())
	if status != RpcOk {
		return result, status
	}
...
// CallPing 
func CallPing(c *Client, ctx context.Context) (errCode RpcErrCode) {
	var buf bytes.Buffer

	body, status := doClient(c, ctx, "/ping", buf.Bytes())
	if status != RpcOk {
		return status
	}
...
// CallPingJunk 
func CallPingJunk(c *Client, ctx context.Context) (errCode RpcErrCode) {
	var buf bytes.Buffer

	body, status := doClient(c, ctx, "/ping_junk", buf.Bytes())
	if status != RpcOk {
		return status
	}
...
// CallPick 
func CallPick(c *Client, ctx context.Context, color Color) (result Color, errCode RpcErrCode) {
	var buf bytes.Buffer
	if !IsColor(color) {
		return result, RpcReqErr
	}
	if err := SetAll(&buf, func(buf *bytes.Buffer) error {
		return SetColor(buf, color)
	}); err != nil {
		return result, RpcReqErr
	}

	body, status := doClient(c, ctx, "/pick", buf.Bytes())
	if status != RpcOk {
		return result, status
	}
...
// CallPickBad 
func CallPickBad(c *Client, ctx context.Context, color Color) (result Color, errCode RpcErrCode) {
	var buf bytes.Buffer
	if !IsColor(color) {
		return result, RpcReqErr
	}
	...
	body, status := doClient(c, ctx, "/pick_bad", buf.Bytes())
	if status != RpcOk {
		return result, status
	}
